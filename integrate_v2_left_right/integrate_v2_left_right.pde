...
void draw() {
  ...
  if (light<=proxSensorThreshold) {
    ...
    rect(300, 125, 600, 250);
    if (t.action == 1 && cursorY > 350) {
      fill(255);
    } else if (t.action == 1) {
      fill(180);
    } else {
...
*/
  ...
  if (light>proxSensorThreshold && heldDown == true && (cursorY < 250 || cursorY > 350)){
    // print("check!");
    text("letgo",90,90);
    int zI = -1;
    if (cursorY < 250) {
